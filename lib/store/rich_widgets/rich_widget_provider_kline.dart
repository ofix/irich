// ///////////////////////////////////////////////////////////////////////////
// Name:        irich/lib/store/rich_widgets/rich_widget_provider_kline.dart
// Purpose:     riverpod provider for kline rich widget
// Author:      songhuabiao
...
// 股票K线控件family数据源参数
class RichWidgetKlineParams {
  final int panelId; // 面板ID
  final int groupId; // 分组ID
  final String shareCode; // 股票代码
  final KlineWndMode wndMode; // 窗口模式
  final KlineType klineType; // K线类型

  RichWidgetKlineParams({
  ...
  bool operator ==(Object other) =>
      ...
      (other is RichWidgetKlineParams &&
          runtimeType == other.runtimeType &&
          panelId == other.panelId &&
          groupId == other.groupId &&
          shareCode == other.shareCode &&
          wndMode == other.wndMode &&
  ...
  @override
  String toString() {
    return 'RichWidgetKlineParams{panelId: $panelId, groupId: $groupId, shareCode: $shareCode, wndMode: $wndMode, klineType: $klineType}';
  }
}

final richWidgetKlineProviders = StateNotifierProvider.autoDispose
    .family<RichWidgetKlineNotifier, KlineCtrlState, RichWidgetKlineParams>((ref, params) {
      final notifier = RichWidgetKlineNotifier(ref: ref, params: params);
      // ✅ 关键修复：主动获取当前值并同步
...
class RichWidgetKlineNotifier extends StateNotifier<KlineCtrlState> {
  // 技术指标映射关系图
  ...
  Ref ref;
  RichWidgetKlineParams params;
  StoreKlines storeKlines = StoreKlines();
  Timer? _timer;

  RichWidgetKlineNotifier({required this.ref, required this.params})
    : super(
        KlineCtrlState(
